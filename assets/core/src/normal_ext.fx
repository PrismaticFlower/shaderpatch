...
void get_normal_gloss(float2 texcoords, float2 detail_texcoords, float3 normal,
                      float3 tangent, float bitangent_sign, out float3 out_normal, 
                      out float out_gloss)
{
   float3 normalTS;
   float gloss;
   ...
   out_normal = perturb_normal(normalTS, normal, tangent, bitangent_sign);
   out_gloss = lerp(1.0, gloss, gloss_map_weight);
}
...
float4 main_ps(Ps_input input) : SV_Target0
{
   ...
   get_normal_gloss(input.diffuse_texcoords, input.detail_texcoords, input.normalWS * input.front_face ? -1.0 : 1.0,
                    input.tangentWS, input.bitangent_sign, normalWS, gloss);
   ...
   // Calculate Lighting
   if (use_specular) {
      color = do_lighting(normalWS, input.positionWS, normalize(view_positionWS - input.positionWS), diffuse_color,
