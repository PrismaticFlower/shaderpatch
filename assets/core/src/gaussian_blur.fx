...
float4 blur_4_ps(float2 texcoords : TEXCOORD) : COLOR
{
   ...
   [unroll] for (int i = 1; i < 3; ++i) {
      color += tex2D(source, texcoords + (offsets[i] * blur_dir_size)).rgb * weights[i];
      color += tex2D(source, texcoords - (offsets[i] * blur_dir_size)).rgb * weights[i];
   }
...
float4 blur_6_ps(float2 texcoords : TEXCOORD) : COLOR
{
...
float4 blur_8_ps(float2 texcoords : TEXCOORD) : COLOR
{
...
float4 blur_12_ps(float2 texcoords : TEXCOORD) : COLOR
{
...
float4 blur_16_ps(float2 texcoords : TEXCOORD) : COLOR
{
...
float4 blur_20_ps(float2 texcoords : TEXCOORD) : COLOR
{
