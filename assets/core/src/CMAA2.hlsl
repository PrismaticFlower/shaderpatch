...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Below is a modification from the original source code to let Shader Patch
// configure CMAA2_EDGE_DETECTION_LUMA_PATH with a single flag.
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#if CMAA2_EDGE_DETECTION_LUMA_SEPARATE_TEXTURE
#define CMAA2_EDGE_DETECTION_LUMA_PATH 2
#else
...
#define CMAA_PACK_SINGLE_SAMPLE_EDGE_TO_HALF_WIDTH                             \
   1 // adds more ALU but reduces memory use for edges by half by packing two 4 bit edge info into one R8_UINT texel - helps on all HW except at really low res
...
// 0 is full color-based edge detection, 1 and 2 are idential log luma based, with
// the difference bing that 1 loads color and computes log luma in-place (less efficient) while 2 loads precomputed log luma from a separate R8_UNORM texture (more efficient).
...
// Optimization paths
#define CMAA2_DEFERRED_APPLY_THREADGROUP_SWAP                                  \
   1 // 1 seems to be better or same on all HW
#define CMAA2_COLLECT_EXPAND_BLEND_ITEMS                                       \
   1 // this reschedules final part of work in the ProcessCandidatesCS (where the sampling and blending takes place) from few to all threads to increase hardware thread occupancy
#ifndef CMAA2_USE_HALF_FLOAT_PRECISION
#define CMAA2_USE_HALF_FLOAT_PRECISION                                         \
   0 // use half precision by default? (not on by default due to driver issues on various different hardware, but let external code decide to define if needed)
#endif
...
#define CMAA2_UAV_STORE_TYPED                                                  \
   1 // use defaults that match the most common scenario: DXGI_FORMAT_R8G8B8A8_UNORM as UAV on a DXGI_FORMAT_R8G8B8A8_UNORM_SRGB resource (no typed stores for sRGB so we have to manually convert)
#endif
...
#define CMAA2_UAV_STORE_CONVERT_TO_SRGB                                        \
   1 // use defaults that match the most common scenario: DXGI_FORMAT_R8G8B8A8_UNORM as UAV on a DXGI_FORMAT_R8G8B8A8_UNORM_SRGB resource (no typed stores for sRGB so we have to manually convert)
#endif
...
#define CMAA2_UAV_STORE_TYPED_UNORM_FLOAT                                      \
   1 // for typed UAV stores: set to 1 for all _UNORM formats and to 0 for _FLOAT formats
#endif
...
#define CMAA2_EXTRA_SHARPNESS                                                  \
   0 // Set to 1 to preserve even more text and shape clarity at the expense of less AA
#endif
//
...
#define CMAA2_STATIC_QUALITY_PRESET                                            \
   2 // 0 - LOW, 1 - MEDIUM, 2 - HIGH, 3 - ULTRA
#endif
...
#define CMAA_MSAA_USE_COMPLEXITY_MASK 1
   ...
   CMAA2_EDGE_DETECTION_LUMA_PATH == 3 || CMAA_MSAA_USE_COMPLEXITY_MASK
SamplerState g_gather_point_clamp_Sampler
   : register(s0); // there's also a slightly less efficient codepath that avoids Gather for easier porting
#endif
...
#if CMAA2_EDGE_UNORM
RWTexture2D<unorm float> g_workingEdges
   : register(u1); // output edges (only used in the fist pass)
#else
RWTexture2D<uint> g_workingEdges
   : register(u1); // output edges (only used in the fist pass)
#endif
...
Texture2D<lpfloat> g_inColorMSComplexityMaskReadonly
   : register(t1); // input MS color control surface
#else
...
#if CMAA_MSAA_SAMPLE_COUNT > 1
   lpfloat3 color =
      g_inColorMSReadonly.Load(int4(pixelPos, sampleIndex, 0), offset).rgb;
#else
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//
void StoreColorSample(uint2 pixelPos, lpfloat3 color, bool isComplexShape,
                      uint msaaSampleIndex)
{
   uint counterIndex;
   g_workingControlBuffer.InterlockedAdd(4 * 12, 1, counterIndex);
   ...
   uint originalIndex;
   InterlockedExchange(g_workingDeferredBlendItemListHeads[quadPos],
                       counterIndexWithHeader, originalIndex);
   g_workingDeferredBlendItemList[counterIndex] =
      uint2(originalIndex, InternalPackColor(color));

   // First one added?
   if (originalIndex == 0xFFFFFFFF) {
      // Make a list of all edge pixels - these cover all potential pixels where AA is applied.
      uint edgeListCounter;
      g_workingControlBuffer.InterlockedAdd(4 * 8, 1, edgeListCounter);
      g_workingDeferredBlendLocationList[edgeListCounter] =
         (quadPos.x << 16) | quadPos.y;
   }
}
//
#if CMAA2_COLLECT_EXPAND_BLEND_ITEMS
#define CMAA2_BLEND_ITEM_SLM_SIZE                                              \
   768 // there's a fallback for extreme cases (observed with this value set to 256 or below) in which case image will remain correct but performance will suffer
...
lpfloat LINEAR_to_SRGB(lpfloat val)
{
   if (val < 0.0031308)
      ...
      val = lpfloat(1.055) * pow(abs(val), lpfloat(1.0) / lpfloat(2.4)) -
            lpfloat(0.055);
   return val;
}
lpfloat3 LINEAR_to_SRGB(lpfloat3 val)
{
   return lpfloat3(LINEAR_to_SRGB(val.x), LINEAR_to_SRGB(val.y),
...
#else
#if CMAA2_UAV_STORE_UNTYPED_FORMAT ==                                          \
   1 // R8G8B8A8_UNORM (or R8G8B8A8_UNORM_SRGB with CMAA2_UAV_STORE_CONVERT_TO_SRGB)
   g_inoutColorWriteonly[pixelPos] = FLOAT4_to_R8G8B8A8_UNORM(lpfloat4(color, 0));
#elif CMAA2_UAV_STORE_UNTYPED_FORMAT ==                                        \
   2 // R10G10B10A2_UNORM (or R10G10B10A2_UNORM_SRGB with CMAA2_UAV_STORE_CONVERT_TO_SRGB)
...
{
   lpfloat2 temp;
   temp.x = EdgeDetectColorCalcDiff(pixelColors[x + y * 3].rgb,
                                    pixelColors[x + 1 + y * 3].rgb);
   temp.y = EdgeDetectColorCalcDiff(pixelColors[x + y * 3].rgb,
                                    pixelColors[x + (y + 1) * 3].rgb);
   return temp; // for HDR edge detection it might be good to premultiply both
                // of these by some factor - otherwise clamping to 1 might prevent some local contrast adaptation. It's a very minor nitpick though, unlikely to significantly affect things.
}
...
lpfloat ComputeLocalContrastV(int x, int y, in lpfloat2 neighbourhood[4][4])
{
   ...
   return max(max(neighbourhood[x + 1][y + 0].y, neighbourhood[x + 1][y + 1].y),
          ...
          lpfloat(g_CMAA2_LocalContrastAdaptationAmount);
...
lpfloat4 ComputeSimpleShapeBlendValues(lpfloat4 edges, lpfloat4 edgesLeft,
                                       lpfloat4 edgesRight, lpfloat4 edgesTop,
                                       lpfloat4 edgesBottom,
                                       uniform bool dontTestShapeValidity)
{
   ...
   lpfloat numberOfEdgesAllAround =
      dot(edgesLeft.bga + edgesRight.rga + edgesTop.rba + edgesBottom.rgb,
          lpfloat3(1, 1, 1));

   // skip if already tested for before calling this function
   if (!dontTestShapeValidity) {
      // No blur for straight edge
      if (numberOfEdges == 1) blurCoeff = 0;

      // L-like step shape ( only blur if it's a corner, not if it's two parallel edges)
      if (numberOfEdges == 2)
         blurCoeff *= ((lpfloat(1.0) - fromBelow * fromAbove) *
                       (lpfloat(1.0) - fromRight * fromLeft));
   }
...
uint LoadEdge(int2 pixelPos, int2 offset, uint msaaSampleIndex)
{
   ...
   uint edge = (uint)(
      g_workingEdges
            .Load(uint2(uint(pixelPos.x + offset.x) / 2, pixelPos.y + offset.y))
            .x *
         255.0 +
      0.5);
#else
...
void GroupsharedLoadQuadHV(uint addr, out lpfloat2 e00, out lpfloat2 e10,
                           out lpfloat2 e01, out lpfloat2 e11)
{
   lpfloat4 valH = g_groupShared2x2FracEdgesH[addr];
   e00.y = valH.x;
   e10.y = valH.y;
   e01.y = valH.z;
   e11.y = valH.w;
   lpfloat4 valV = g_groupShared2x2FracEdgesV[addr];
   e00.x = valV.x;
   e10.x = valV.y;
   e01.x = valV.z;
   e11.x = valV.w;
...
//
[numthreads(CMAA2_CS_INPUT_KERNEL_SIZE_X, CMAA2_CS_INPUT_KERNEL_SIZE_Y, 1)] void
EdgesColor2x2CS(uint3 groupID
                : SV_GroupID, uint3 groupThreadID
                : SV_GroupThreadID) {
   // screen position in the input (expanded) kernel (shifted one 2x2 block up/left)
   uint2 pixelPos = groupID.xy * int2(CMAA2_CS_OUTPUT_KERNEL_SIZE_X,
                                      CMAA2_CS_OUTPUT_KERNEL_SIZE_Y) +
                    groupThreadID.xy - int2(1, 1);
   pixelPos *= int2(2, 2);
   ...
   const bool inOutputKernel =
      !any(bool4(groupThreadID.x == (CMAA2_CS_INPUT_KERNEL_SIZE_X - 1),
                 groupThreadID.x == 0,
                 groupThreadID.y == (CMAA2_CS_INPUT_KERNEL_SIZE_Y - 1),
                 groupThreadID.y == 0));
...
#if CMAA_MSAA_USE_COMPLEXITY_MASK
   {
      ...
      float4 TL =
         g_inColorMSComplexityMaskReadonly.GatherRed(g_gather_point_clamp_Sampler,
                                                     gatherUV, int2(0, 0));
      float4 TR =
         g_inColorMSComplexityMaskReadonly.GatherRed(g_gather_point_clamp_Sampler,
                                                     gatherUV, int2(2, 0));
   ...
   [loop] for (uint msaaSampleIndex = 0;
               msaaSampleIndex < CMAA_MSAA_SAMPLE_COUNT; msaaSampleIndex++)
   {
      bool msaaSampleIsRelevant = !firstLoopIsEnough || msaaSampleIndex == 0;
      [branch] if (msaaSampleIsRelevant)
      {
#else
   {
      uint msaaSampleIndex = 0;
         ...
         [unroll] for (i = 0; i < 3 * 3 - 1; i++) pixelColors[i] =
            LoadSourceColor(pixelPos, int2(i % 3, i / 3), msaaSampleIndex).rgb;

         [unroll] for (i = 0; i < 3 * 3 - 1; i++) pixelColors[i] =
            ProcessColorForEdgeDetect(pixelColors[i]);
      ...
      float4 TR = g_inLumaReadonly.GatherRed(g_gather_point_clamp_Sampler,
                                             gatherUV, int2(1, 0));
      float4 BL = g_inLumaReadonly.GatherRed(g_gather_point_clamp_Sampler,
                                             gatherUV, int2(0, 1));
      pixelLumas[0] = TL.w;
      pixelLumas[1] = TL.z;
      pixelLumas[2] = TR.z;
      ...
      float2 gatherUV = (float2(pixelPos) + float2(0.5, 0.5)) / texSize;
      float4 TL =
         g_inoutColorReadonly.GatherAlpha(g_gather_point_clamp_Sampler, gatherUV);
      float4 TR = g_inoutColorReadonly.GatherAlpha(g_gather_point_clamp_Sampler,
                                                   gatherUV, int2(1, 0));
      float4 BL = g_inoutColorReadonly.GatherAlpha(g_gather_point_clamp_Sampler,
                                                   gatherUV, int2(0, 1));
      pixelLumas[0] = (lpfloat)TL.w;
      pixelLumas[1] = (lpfloat)TL.z;
      pixelLumas[2] = (lpfloat)TR.z;
      pixelLumas[3] = (lpfloat)TL.x;
      pixelLumas[4] = (lpfloat)TL.y;
      pixelLumas[5] = (lpfloat)TR.y;
      pixelLumas[6] = (lpfloat)BL.x;
      pixelLumas[7] = (lpfloat)BL.y;
#endif
      ...
      qe3 = ComputeEdgeLuma(1, 1, pixelLumas);
         ...
         g_groupShared2x2FracEdgesV[centerAddr2x2 + rowStride2x2 * 0] =
            lpfloat4(qe0.x, qe1.x, qe2.x, qe3.x);
         g_groupShared2x2FracEdgesH[centerAddr2x2 + rowStride2x2 * 0] =
            lpfloat4(qe0.y, qe1.y, qe2.y, qe3.y);
      ...
      [branch] if (inOutputKernel)
      {
         lpfloat2 topRow =
            g_groupShared2x2FracEdgesH[centerAddr2x2 - rowStride2x2].zw; // top row's bottom edge
         lpfloat2 leftColumn = g_groupShared2x2FracEdgesV[centerAddr2x2 - 1].yw; // left column's right edge

         bool someNonZeroEdges =
            any(lpfloat4(qe0, qe1) + lpfloat4(qe2, qe3) +
                lpfloat4(topRow[0], topRow[1], leftColumn[0], leftColumn[1]));
         // bool someNonZeroEdges = packedCenterEdges.x | packedCenterEdges.y | (packedQuadP0M1.y & 0xFFFF0000) | (packedQuadM1P0.x & 0xFF00FF00);

         [branch] if (someNonZeroEdges)
         {
#if CMAA_MSAA_SAMPLE_COUNT == 1
            ...
            GroupsharedLoadQuadHV(centerAddr2x2 - rowStride2x2 - 1, dummyd0,
                                  dummyd1, dummyd2, neighbourhood[0][0]);
            GroupsharedLoadQuadHV(centerAddr2x2 - rowStride2x2, dummyd0, dummyd1,
                                  neighbourhood[1][0], neighbourhood[2][0]);
            GroupsharedLoadQuadHV(centerAddr2x2 - rowStride2x2 + 1, dummyd0,
                                  dummyd1, neighbourhood[3][0], dummyd2);
            GroupsharedLoadQuadHV(centerAddr2x2 - 1, dummyd0, neighbourhood[0][1],
                                  dummyd1, neighbourhood[0][2]);
            GroupsharedLoadQuadHV(centerAddr2x2 + 1, neighbourhood[3][1],
                                  dummyd0, neighbourhood[3][2], dummyd1);
            GroupsharedLoadQuadHV(centerAddr2x2 - 1 + rowStride2x2, dummyd0,
                                  neighbourhood[0][3], dummyd1, dummyd2);
            GroupsharedLoadQuadHV(centerAddr2x2 + rowStride2x2, neighbourhood[1][3],
                                  neighbourhood[2][3], dummyd0, dummyd1);
            ...
            topRow[0] = (topRow[0] - ComputeLocalContrastH(0, -1, neighbourhood)) >
                        GetActualEdgeThreshold();
            topRow[1] = (topRow[1] - ComputeLocalContrastH(1, -1, neighbourhood)) >
                        GetActualEdgeThreshold();
            leftColumn[0] =
               (leftColumn[0] - ComputeLocalContrastV(-1, 0, neighbourhood)) >
               GetActualEdgeThreshold();
            leftColumn[1] =
               (leftColumn[1] - ComputeLocalContrastV(-1, 1, neighbourhood)) >
               GetActualEdgeThreshold();

            ce[0].x = (qe0.x - ComputeLocalContrastV(0, 0, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[0].y = (qe0.y - ComputeLocalContrastH(0, 0, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[1].x = (qe1.x - ComputeLocalContrastV(1, 0, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[1].y = (qe1.y - ComputeLocalContrastH(1, 0, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[2].x = (qe2.x - ComputeLocalContrastV(0, 1, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[2].y = (qe2.y - ComputeLocalContrastH(0, 1, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[3].x = (qe3.x - ComputeLocalContrastV(1, 1, neighbourhood)) >
                      GetActualEdgeThreshold();
            ce[3].y = (qe3.y - ComputeLocalContrastH(1, 1, neighbourhood)) >
                      GetActualEdgeThreshold();
#else
            ...
            [unroll] for (i = 0; i < 4; i++)
            {
               ...
               bool isCandidate = (edges.x * edges.y + edges.y * edges.z +
                                   edges.z * edges.w + edges.w * edges.x) != 0;
               if (isCandidate) {
                  uint counterIndex;
                  g_workingControlBuffer.InterlockedAdd(4 * 4, 1, counterIndex);
                  g_workingShapeCandidates[counterIndex] =
                     (localPixelPos.x << 18) | (msaaSampleIndex << 14) |
                     localPixelPos.y;
               }
...
#if CMAA2_EDGE_UNORM
      g_workingEdges[int2(pixelPos.x / 2, pixelPos.y + 0)] =
         ((outEdges[1] << 4) | outEdges[0]) / 255.0;
      g_workingEdges[int2(pixelPos.x / 2, pixelPos.y + 1)] =
         ((outEdges[3] << 4) | outEdges[2]) / 255.0;
#else
      g_workingEdges[int2(pixelPos.x / 2, pixelPos.y + 0)] =
         (outEdges[1] << 4) | outEdges[0];
      g_workingEdges[int2(pixelPos.x / 2, pixelPos.y + 1)] =
         (outEdges[3] << 4) | outEdges[2];
#endif
#else
      {
...
[numthreads(1, 1, 1)] void ComputeDispatchArgsCS(uint3 groupID
                                                 : SV_GroupID)
{
   ...
   if (groupID.x == 1) {
      ...
      // check for overflow!
      uint appendBufferMaxCount;
      uint appendBufferStride;
      ...
      g_workingControlBuffer.Store(4 * 0, (shapeCandidateCount +
                                           CMAA2_PROCESS_CANDIDATES_NUM_THREADS - 1) /
   ...
   // activated once on Dispatch( 1, 2, 1 )
      ...
      // check for overflow!
      {
         uint appendBufferMaxCount;
         uint appendBufferStride;
         g_workingDeferredBlendLocationList.GetDimensions(appendBufferMaxCount,
                                                          appendBufferStride);
      ...
      g_workingControlBuffer.Store(4 * 1, (blendLocationCount +
                                           CMAA2_DEFERRED_APPLY_NUM_THREADS - 1) /
                                             CMAA2_DEFERRED_APPLY_NUM_THREADS);
#else
      g_workingControlBuffer.Store(4 * 0, (blendLocationCount +
                                           CMAA2_DEFERRED_APPLY_NUM_THREADS - 1) /
                                             CMAA2_DEFERRED_APPLY_NUM_THREADS);
      g_workingControlBuffer.Store(4 * 1, 1);
#endif
...
void FindZLineLengths(out lpfloat lineLengthLeft, out lpfloat lineLengthRight,
                      uint2 screenPos, uniform bool horizontal,
                      uniform bool invertedZShape, const float2 stepRight,
                      uint msaaSampleIndex)
{
...
#define CMAA2_EXTRA_CONSERVATIVENESS2 0
   ...
   [loop] for (;;)
   {
      uint edgeLeft = LoadEdge(screenPos.xy - stepRight * float(lineLengthLeft),
                               int2(0, 0), msaaSampleIndex);
      uint edgeRight =
         LoadEdge(screenPos.xy + stepRight * (float(lineLengthRight) + 1),
                  int2(0, 0), msaaSampleIndex);
      ...
      if (!continueLeft && !continueRight) maxLR = (lpfloat)c_maxLineLength;

         // either the longer one is ahead of the smaller (already stopped) one
         // by more than a factor of x, or both are stopped - end the search.
#if CMAA2_EXTRA_SHARPNESS
      if (maxLR >= min((lpfloat)c_maxLineLength,
                       (1.20 * min(lineLengthRight, lineLengthLeft) - 0.20)))
#else
      if (maxLR >= min((lpfloat)c_maxLineLength,
...
#if CMAA2_COLLECT_EXPAND_BLEND_ITEMS
bool CollectBlendZs(uint2 screenPos, bool horizontal, bool invertedZShape,
                    lpfloat shapeQualityScore, lpfloat lineLengthLeft,
                    lpfloat lineLengthRight, float2 stepRight, uint msaaSampleIndex)
{
   ...
   lpfloat dampenEffect =
      saturate(lpfloat(lineLengthLeft + lineLengthRight - shapeQualityScore) *
               c_dampeningEffect);
   ...
   for (lpfloat i = loopFrom; i <= loopTo; i++) {
      ...
      encodedItem.y = itemValStatic | ((uint(i + 256) /*& 0x3FF*/) << 20) |
                      ((uint(srcOffset + 256) /*& 0x3FF*/) << 10) |
                      uint(saturate(lerpK) * 1023 + 0.5);
...
void BlendZs(uint2 screenPos, bool horizontal, bool invertedZShape,
             lpfloat shapeQualityScore, lpfloat lineLengthLeft,
             lpfloat lineLengthRight, float2 stepRight, uint msaaSampleIndex)
{
   ...
   for (lpfloat i = loopFrom; i <= loopTo; i++) {
      ...
      lpfloat3 colorCenter =
         LoadSourceColor(pixelPos, int2(0, 0), msaaSampleIndex).rgb;
      lpfloat3 colorFrom =
         LoadSourceColor(pixelPos.xy + blendDir * float(srcOffset).xx,
                         int2(0, 0), msaaSampleIndex)
            .rgb;
...
void DetectZsHorizontal(in lpfloat4 edges, in lpfloat4 edgesM1P0,
                        in lpfloat4 edgesP1P0, in lpfloat4 edgesP2P0,
                        out lpfloat invertedZScore, out lpfloat normalZScore){
   // Inverted Z case:
   //   __
   //  X|
   // ¯¯
...
[numthreads(CMAA2_PROCESS_CANDIDATES_NUM_THREADS, 1, 1)] void ProcessCandidatesCS(
   uint3 dispatchThreadID
   : SV_DispatchThreadID, uint3 groupThreadID
   : SV_GroupThreadID) {
#if CMAA2_COLLECT_EXPAND_BLEND_ITEMS
   if (groupThreadID.x == 0) g_groupSharedBlendItemCount = 0;
   GroupMemoryBarrierWithGroupSync();
#endif
   ...
   if (dispatchThreadID.x < numCandidates) {
    ...
    return;
      ...
      lpfloat4 edgesLeft =
         UnpackEdgesFlt(LoadEdge(pixelPos, int2(-1, 0), msaaSampleIndex));
      lpfloat4 edgesRight =
         UnpackEdgesFlt(LoadEdge(pixelPos, int2(1, 0), msaaSampleIndex));
      lpfloat4 edgesBottom =
         UnpackEdgesFlt(LoadEdge(pixelPos, int2(0, 1), msaaSampleIndex));
      lpfloat4 edgesTop =
         UnpackEdgesFlt(LoadEdge(pixelPos, int2(0, -1), msaaSampleIndex));
      ...
      {
         lpfloat4 blendVal =
            ComputeSimpleShapeBlendValues(edges, edgesLeft, edgesRight,
                                          edgesTop, edgesBottom, true);

         const lpfloat fourWeightSum = dot(blendVal, lpfloat4(1, 1, 1, 1));
         const lpfloat centerWeight = 1.0 - fourWeightSum;

         lpfloat3 outColor =
            LoadSourceColor(pixelPos, int2(0, 0), msaaSampleIndex).rgb * centerWeight;
         [flatten] if (blendVal.x > 0.0) // from left
         {
            lpfloat3 pixelL =
               LoadSourceColor(pixelPos, int2(-1, 0), msaaSampleIndex).rgb;
            outColor.rgb += blendVal.x * pixelL;
         }
         [flatten] if (blendVal.y > 0.0) // from above
         {
            lpfloat3 pixelT =
               LoadSourceColor(pixelPos, int2(0, -1), msaaSampleIndex).rgb;
            outColor.rgb += blendVal.y * pixelT;
         }
         [flatten] if (blendVal.z > 0.0) // from right
         {
            lpfloat3 pixelR =
               LoadSourceColor(pixelPos, int2(1, 0), msaaSampleIndex).rgb;
            outColor.rgb += blendVal.z * pixelR;
         }
         [flatten] if (blendVal.w > 0.0) // from below
         {
            lpfloat3 pixelB =
               LoadSourceColor(pixelPos, int2(0, 1), msaaSampleIndex).rgb;
      ...
      // complex shapes - detect
      {
         ...
         // horizontal
         {
            ...
            lpfloat4 edgesP2P0 =
               UnpackEdgesFlt(LoadEdge(pixelPos, int2(2, 0), msaaSampleIndex));

            DetectZsHorizontal(edges, edgesM1P0, edgesP1P0, edgesP2P0,
         ...
         // vertical
         {
            ...
            lpfloat4 edgesP2P0 =
               UnpackEdgesFlt(LoadEdge(pixelPos, int2(0, -2), msaaSampleIndex));

            DetectZsHorizontal(edges.argb, edgesM1P0.argb, edgesP1P0.argb,
                               edgesP2P0.argb, invertedZScore, normalZScore);
         ...
         if (maxScore > 0) {
            ...
            FindZLineLengths(lineLengthLeft, lineLengthRight, pixelPos,
                             horizontal, invertedZ, stepRight, msaaSampleIndex);
            ...
            if ((lineLengthLeft + lineLengthRight) >= (5.0)) {
               ...
               if (!CollectBlendZs(pixelPos, horizontal, invertedZ,
                                   shapeQualityScore, lineLengthLeft,
                                   lineLengthRight, stepRight, msaaSampleIndex))
#endif
                  BlendZs(pixelPos, horizontal, invertedZ, shapeQualityScore,
                          lineLengthLeft, lineLengthRight, stepRight, msaaSampleIndex);
            }
         }
      }
   }
   ...
   const uint loops =
      ((int)g_groupSharedBlendItemCount +
       (int)CMAA2_PROCESS_CANDIDATES_NUM_THREADS - 1 - groupThreadID.x) /
      CMAA2_PROCESS_CANDIDATES_NUM_THREADS;
   ...
   for (uint loop = 0; loop < loops; loop++) {
      ...
      lpfloat2 itemStepRight =
         (itemHorizontal) ? (lpfloat2(1, 0)) : (lpfloat2(0, -1));
      lpfloat2 itemBlendDir =
         (itemHorizontal) ? (lpfloat2(0, -1)) : (lpfloat2(-1, 0));
      if (itemInvertedZ) itemBlendDir = -itemBlendDir;

      uint2 itemPixelPos = startingPos + itemStepRight * lpfloat(itemStepIndex);

      lpfloat3 colorCenter =
         LoadSourceColor(itemPixelPos, int2(0, 0), itemMSAASampleIndex).rgb;
      lpfloat3 colorFrom =
         LoadSourceColor(itemPixelPos.xy + itemBlendDir * lpfloat(itemSrcOffset).xx,
                         int2(0, 0), itemMSAASampleIndex)
            .rgb;
...
#endif
   void DeferredColorApply2x2CS(uint3 dispatchThreadID
                                : SV_DispatchThreadID, uint3 groupThreadID
                                : SV_GroupThreadID)
{
   ...
   [unroll] for (uint msaaSampleIndex = 0;
                 msaaSampleIndex < CMAA_MSAA_SAMPLE_COUNT; msaaSampleIndex++)
   ...
   const uint maxLoops =
      32 * CMAA_MSAA_SAMPLE_COUNT; // do the loop to prevent bad data hanging the GPU <- probably not needed
   {
      for (uint i = 0; (counterIndexWithHeader != 0xFFFFFFFF) && (i < maxLoops); i++) {
         ...
         uint2 val =
            g_workingDeferredBlendItemList[counterIndexWithHeader & ((1 << 26) - 1)];
...
#if CMAA_MSAA_SAMPLE_COUNT > 1
      lpfloat4 outColor = 0;
      for (uint msaaSampleIndex = 0; msaaSampleIndex < CMAA_MSAA_SAMPLE_COUNT;
           msaaSampleIndex++) {
         if (outColors[msaaSampleIndex].a != 0)
            outColor.xyz +=
               outColors[msaaSampleIndex].rgb / (outColors[msaaSampleIndex].a);
         else
...
[numthreads(16, 16, 1)] void DebugDrawEdgesCS(uint2 dispatchThreadID
                                              : SV_DispatchThreadID) {
   int msaaSampleIndex = 0;
   lpfloat4 edges =
      UnpackEdgesFlt(LoadEdge(dispatchThreadID, int2(0, 0), msaaSampleIndex));

   // show MSAA control mask
   // uint v = g_inColorMSComplexityMaskReadonly.Load( int3( dispatchThreadID, 0
   // ) ); FinalUAVStore( dispatchThreadID, float3( v, v, v ) ); return;
