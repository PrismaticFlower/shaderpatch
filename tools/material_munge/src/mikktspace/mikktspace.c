...
tbool genTangSpace(const SMikkTSpaceContext* pContext, const float fAngularThreshold)
{
   ...
   iNrActiveGroups = Build4RuleGroups(pTriInfos, pGroups, piGroupTrianglesBuffer,
   ...
   for (f = 0; f < iNrFaces; f++) {
      ...
      for (i = 0; i < verts; i++) {
         ...
         if (pContext->m_pInterface->m_setTSpaceBasic != NULL)
            pContext->m_pInterface->m_setTSpaceBasic(pContext, tang,
                                                     pTSpace->bOrient == TTRUE
                                                        ? 1.0f
                                                        : (-1.0f),
                                                     f, i);
   ...
   while (t < (iNrTrianglesIn - 1)) {
      ...
      if (iFO_a == iFO_b) // this is a quad
      {
         ...
         const tbool bIsDeg_b =
            (pTriInfos[t + 1].iFlag & MARK_DEGENERATE) != 0 ? TTRUE : TFALSE;
      ...
      for (i = 0; i < 3; i++) {
         ...
         if ((pTriInfos[f].iFlag & GROUP_WITH_ANY) == 0 &&
            ...
            if (neigh_indexL >= 0) // neighbor
            {
               ...
               const tbool bOrPre2 =
                  (pTriInfos[neigh_indexL].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE
                                                                           : TFALSE;
            ...
            if (neigh_indexR >= 0) // neighbor
            {
               ...
               const tbool bOrPre2 =
                  (pTriInfos[neigh_indexR].iFlag & ORIENT_PRESERVING) != 0 ? TTRUE
                                                                           : TFALSE;
               const tbool bDiff = bOrPre != bOrPre2 ? TTRUE : TFALSE;
               assert(bAnswer || bDiff);
               (void)bAnswer, (void)bDiff; /* quiet warnings in non debug mode */
   ...
   for (g = 0; g < iNrActiveGroups; g++) {
      ...
      for (i = 0; i < pGroup->iNrFaces; i++) // triangles
      {
         ...
         for (j = 0; j < pGroup->iNrFaces; j++) {
            ...
            {
               const tbool bAny =
                  ((pTriInfos[f].iFlag | pTriInfos[t].iFlag) & GROUP_WITH_ANY) != 0
                     ? TTRUE
                     : TFALSE;
