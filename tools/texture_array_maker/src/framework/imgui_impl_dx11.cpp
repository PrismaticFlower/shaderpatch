                                            ...
                                            ID3D11DeviceContext* ctx)
{
   ...
   ctx->IASetIndexBuffer(bd->pIB, sizeof(ImDrawIdx) == 2 ? DXGI_FORMAT_R16_UINT : DXGI_FORMAT_R32_UINT,
                         0);
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
   ...
   if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount) {
      ...
      desc.MiscFlags = 0;
      if (bd->pd3dDevice->CreateBuffer(&desc, NULL, &bd->pVB) < 0) return;
   }
   if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount) {
      ...
      desc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
      if (bd->pd3dDevice->CreateBuffer(&desc, NULL, &bd->pIB) < 0) return;
   }
   ...
   // Create the vertex shader
   {
      static const char* vertexShader = "cbuffer vertexBuffer : register(b0) \
            ...
            {\
              ...
              output.col = float4(pow(input.col.rgb, 2.2), input.col.a);\
              output.uv  = input.uv;\
              return output;\
            }";
