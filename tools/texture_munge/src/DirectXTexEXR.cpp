...
#pragma warning(push)
#pragma warning(disable : 4244 4996 5033 5040)
...
public:
   ...
   ~auto_delete_file()
   {
      if (m_handle) {
         ...
         (void)SetFileInformationByHandle(m_handle, FileDispositionInfo, &info,
                                          sizeof(info));
      }
   }
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::GetMetadataFromEXRFile(const wchar_t* szFile,
                                                               TexMetadata& metadata)
{
   if (!szFile) return E_INVALIDARG;

   char fileName[MAX_PATH];
   int result = WideCharToMultiByte(CP_UTF8, 0, szFile, -1, fileName, MAX_PATH,
                                    nullptr, nullptr);
   if (result <= 0) {
      *fileName = 0;
   }
...
#else
   ScopedHandle hFile(
      safe_handle(CreateFileW(szFile, GENERIC_READ, FILE_SHARE_READ, nullptr,
   ...
   try {
      ...
      if (width < 1 || height < 1) return E_FAIL;
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::LoadFromEXRFile(const wchar_t* szFile,
                                                        TexMetadata* metadata,
                                                        ScratchImage& image)
{
   if (!szFile) return E_INVALIDARG;
   ...
   try {
      ...
      file.setFrameBuffer(reinterpret_cast<Imf::Rgba*>(image.GetPixels()) -
...
//-------------------------------------------------------------------------------------
_Use_decl_annotations_ HRESULT DirectX::SaveToEXRFile(const Image& image,
                                                      const wchar_t* szFile)
{
   if (!szFile) return E_INVALIDARG;

   if (!image.pixels) return E_POINTER;
   ...
   case DXGI_FORMAT_R16G16B16A16_FLOAT:
      if ((image.rowPitch % 8) > 0) return E_FAIL;
      break;
...
#if (_WIN32_WINNT >= _WIN32_WINNT_WIN8)
   ScopedHandle hFile(
      safe_handle(CreateFile2(szFile, GENERIC_WRITE, 0, CREATE_ALWAYS, nullptr)));
#else
   ScopedHandle hFile(safe_handle(CreateFileW(szFile, GENERIC_WRITE, 0, nullptr,
   ...
   try {
      ...
      if (image.format == DXGI_FORMAT_R16G16B16A16_FLOAT) {
         file.setFrameBuffer(reinterpret_cast<const Imf::Rgba*>(image.pixels),
                             1, image.rowPitch / 8);
         file.writePixels(height);
      }
      else {
         std::unique_ptr<XMHALF4> temp(new (std::nothrow) XMHALF4[width * height]);
         if (!temp) return E_OUTOFMEMORY;
