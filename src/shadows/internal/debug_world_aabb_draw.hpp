...
struct Debug_world_aabb_draw {
   ...
   Debug_world_aabb_draw(ID3D11Device& device);

   void draw(ID3D11DeviceContext2& dc, const World_inputs& world,
             const glm::mat4& projection_matrix, const Target_inputs& target) noexcept;

   void draw(ID3D11DeviceContext2& dc, const World_inputs& world,
             const float camera_yaw, const float camera_pitch,
             const glm::vec3& camera_positioWS, const Target_inputs& target) noexcept;
