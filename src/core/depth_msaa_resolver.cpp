...
Depth_msaa_resolver::Depth_msaa_resolver(ID3D11Device5& device,
                                         shader::Database& shader_database) noexcept
   : _vs{std::get<0>(
        shader_database.vertex("depth msaa resolve"sv).entrypoint("main_vs"sv, 0))},
     _ps_x2{shader_database.pixel("depth msaa resolve"s).entrypoint("main_x2_ps"s)},
     _ps_x4{shader_database.pixel("depth msaa resolve"s).entrypoint("main_x4_ps"s)},
     _ps_x8{shader_database.pixel("depth msaa resolve"s).entrypoint("main_x8_ps"s)},
     _ds_state{[&] {
...
auto Depth_msaa_resolver::get_pixel_shader(const UINT sample_count) const noexcept
   -> ID3D11PixelShader*
{
