...
#include "imgui.h"
#include "imgui_impl_dx11_shaders.hpp"
   ...
   for (int n = 0; n < draw_data->CmdListsCount; n++) {
      ...
      for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++) {
         ...
         if (pcmd->UserCallback) {
            ...
            const D3D11_RECT r = {(LONG)(pcmd->ClipRect.x - pos.x),
                                  (LONG)(pcmd->ClipRect.y - pos.y),
                                  (LONG)(pcmd->ClipRect.z - pos.x),
                                  (LONG)(pcmd->ClipRect.w - pos.y)};
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
   ...
   {
      [[maybe_unused]] static const char* vertexShader = "cbuffer vertexBuffer : register(b0) \
            {\
      ...
      if (g_pd3dDevice->CreateVertexShader((DWORD*)imgui_impl_dx11_vs_shader,
                                           sizeof(imgui_impl_dx11_vs_shader),
      ...
      if (g_pd3dDevice->CreateInputLayout(local_layout, 3, imgui_impl_dx11_vs_shader,
                                          sizeof(imgui_impl_dx11_vs_shader),
                                          &g_pInputLayout) != S_OK)
         return false;
   ...
   {
      [[maybe_unused]] static const char* pixelShader = "struct PS_INPUT\
            {\
            float4 pos : SV_POSITION;\
            float4 col : COLOR0;\
            float2 uv  : TEXCOORD0;\
      ...
      if (g_pd3dDevice->CreatePixelShader((DWORD*)imgui_impl_dx11_ps_shader,
                                          sizeof(imgui_impl_dx11_ps_shader),
...
bool ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
   ...
   if (device->QueryInterface(IID_PPV_ARGS(&pDXGIDevice)) != S_OK) return false;
   if (pDXGIDevice->GetParent(IID_PPV_ARGS(&pDXGIAdapter)) != S_OK)
      return false;
   if (pDXGIAdapter->GetParent(IID_PPV_ARGS(&pFactory)) != S_OK) return false;
