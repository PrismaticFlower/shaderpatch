                                    ...
                                    const D3DBOX* box, DWORD flags) noexcept
{
   ...
   _scratch_buffer.ensure_min_size(_resource_size);

   locked_box->RowPitch = _width;
   locked_box->SlicePitch = _width * _height;
   locked_box->pBits = _scratch_buffer.data();
...
HRESULT Texture3d_resource::UnlockBox(UINT level) noexcept
{
   ...
   _scratch_buffer.ensure_within_size(max_persist_buffer_size);
...
void Texture3d_resource::create_resource() noexcept
{
   const auto volume_res_header = bit_cast<Volume_resource_header>(
      gsl::span{_scratch_buffer.data(), sizeof(Volume_resource_header)});
   ...
   const auto payload =
      gsl::make_span(_scratch_buffer.data() + sizeof(Volume_resource_header),
